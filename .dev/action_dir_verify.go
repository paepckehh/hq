...
func verifyMAP(id *HQ) (uint64, uint64, uint64) {
	...
	for i := 0; i < id.IO.CPU; i++ {
		go func() {
			for t := range chan_feed {
				...
				if err != nil {
					// pre existing hash of an [broken] symlink
					if t.checksum == "ac169ead597dac88b2d7223edd85c9895392532cfc7a3c5c29a3fbe3ccba37f2" {
						f, err := os.Lstat(t.filename)
						switch {
						case err != nil: // removed now
							chan_fail <- failed{filename: t.filename, reason: 1}
						case f.Mode()&fs.ModeSymlink != 0: // no change
	...
	// collect current filesystem
	go func() {
		l := recursiveFileList(id.IO.DirName, 0)
		chan_curr <- l
		close(chan_curr)
	}()

	// collect chan_found, diff against curr
	go func() {
		if id.IO.ColorUI {
